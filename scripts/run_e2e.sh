...
# Usage: ./scripts/run_e2e.sh <app_name> [test_file]
#    or: ./scripts/run_e2e.sh <test_file> (defaults to admin_app)
...
if [[ "$ARG1" == *".yaml" ]]; then
  # Usage: ./scripts/run_e2e.sh tests/some_test.yaml
  # Default to admin_app if only a yaml file is provided
  APP_NAME="admin_app"
  MANUAL_TEST_FILE="$ARG1"
  echo "⚠️  No app name specified. Defaulting to '$APP_NAME' and running '$MANUAL_TEST_FILE'."
else
  APP_NAME=$ARG1
  MANUAL_TEST_FILE=$ARG2
fi

if [ -z "$APP_NAME" ]; then
  echo "❌ Error: App name argument is required."
  echo "Usage: ./scripts/run_e2e.sh <app_name> [optional_test_file.yaml]"
  echo "   or: ./scripts/run_e2e.sh <test_file.yaml>"
...
if [ -n "$MANUAL_TEST_FILE" ]; then
  TEST_FILES=("$MANUAL_TEST_FILE")
else
  case $APP_NAME in
    admin_app)
...
if [ -n "$MANUAL_TEST_FILE" ]; then
  echo "🎯 Target Test: $MANUAL_TEST_FILE"
fi
