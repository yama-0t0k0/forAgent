...
| :--- | :--- | :--- | :--- |
| **public_profile** | 個人ユーザー（公開情報） | ユーザー一覧表示、検索、基本統計 | DashboardScreen |
| **private_info** | 個人ユーザー（非公開情報） | 詳細確認、PII管理（氏名・連絡先等） | UserDetailModal |
| **individual** (Legacy) | 個人ユーザー（旧） | 移行期間中の参照（`public_profile` / `private_info` へ移行中） | - |
...
- **User**: `shared/common_frontend/src/core/models/User.js`
  - `public_profile` と `private_info` の結合・分離を隠蔽し、統一的なインターフェースを提供します。
...
```mermaid
graph LR
    A1["Firestore (public_profile)"]
    A2["Firestore (private_info)"]
    B["Firestore (fmjs)"]
    ...
    A1 -->|getDocs| Wrapper
    A2 -->|getDocs (Admin auth)| Wrapper
    B -->|getDocs| Wrapper
    
    Wrapper -->|setInitialData (Merged User)| Context
