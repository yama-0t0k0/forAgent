...
### 3.1 招待コード発行権限と制約条件
本サービスは完全招待制であり、未登録の「非ユーザー」を招待する権限は厳格に管理される。

- **Admin（システム管理者）**:
  - 最初から未登録の「非ユーザー」に対する完全な招待権限を持つ。
  - 発行可能な招待コード種別: **「法人用」「個人用」の両方**。
- **個人ユーザー**:
  - デフォルトでは招待権限を持たない。
  - Adminから「招待権限」を付与（許可）された場合のみ、未登録の「非ユーザー」に対して招待コードを発行できる。
  - 発行可能な招待コード種別: **「個人用」のみ**。

### 3.2 法人アカウント（企業）の新規作成フロー
法人の新規登録（登記）は、ユーザーの登録状態によってフローが異なる。

1. **未登録の非ユーザーが「法人用」招待コードを受け取った場合**:
   - まず、自らの「個人ユーザー（Individual）」としての登録を完了させる。
   - 登録が完了した後に、続けて自らが所属する企業の「法人ユーザー」新規登録（作成）を行う。
   - 法人の作成が完了した時点で、そのユーザーは暫定的に自社の「 Alpha（採用管理者）」ロールに就く。
   - Alpha となったユーザーは、自社の社員に対して「α：採用管理者」「β：採用関係者」「γ：一般社員」の 3 つのロールを割り当てることができる。
2. **すでに当サービスに登録済みのユーザーが法人を新規作成したい場合**:
   - 既存ユーザーは、自身の操作のみで自由に企業を新規作成することはできない。
   - 運営（Admin）に事前に連絡し、手動で「採用管理者（Alpha）」として登録するための事前権限付与を依頼する必要がある。
   - Adminからの付与が完了すると、自らのマイページに「法人アカウントを新規作成する」ボタンが表示され、法人登録に進むことができる。
...
2. **最後1人の Alpha 保護 (Lockout Prevention)**: 
   - 社内に Alpha が 1 名しかいない状態で、そのユーザーを「削除」「降格（β/γへの変更）」「退会」させようとした場合、システムはこれを強力にブロックする。
   - **UIアラートと後任指定の強制**: 
     - 実行時には「管理者が1名しかいないため、この操作は実行できません」という警告アラートを表示する。
     - 操作を続行するには、事前に「別のメンバーを Alpha に昇格させる」か、あるいは「権限委譲（Successor Designation）」フローを完遂させてから交代することを具体的に促す。
3. **冗長化勧告通知**: Alpha が 1 名のみの場合、ガバナンスの安全性向上のため、通知ベル等で定期的に複数名（推奨2名以上）管理体制への移行をレコメンドする。
...
### Phase 3: Verification & Fine-tuning (完了)
- [x] シミュレータ環境での E2E 動作検証
- [x] 例外処理 (期限切れコード等) のハンドリング強化
- [x] セキュリティルール (jest) による自動テストの拡充
...
### 9.1 法人登録の「1回限り」制限
1つの個人アカウントから作成できる法人アカウントは、原則として **1社のみ** とする。（転職して新しい会社で法人アカウントを作りたい場合は、また同じフローとなるので、その旨を運営に伝えてください）
...
| ロール名 | UI表記 | 定義・権限 |
| :--- | :--- | :--- |
| `corporate-alpha` | **α：採用管理者** | 法人アカウントの設立者及び、法人アカウントの設立者からalphaロールを付与されたユーザー。企業情報の編集、メンバー招待、全採用データの閲覧が可能。 |
...
## 10. ロール変更通知システムとガバナンス (Milestone 10)

透明性の高い組織運営を支援するため、法人のロール変更が行われた際の通知ロジックを以下のように定義する。

### 10.1 通知の対象とタイミング
法人の管理者（Alpha）がメンバーのロールを変更（昇格・降格）した際、即座に以下のユーザーへ通知を送信する。
1. **対象者本人**: 自身のロールが変更されたことを覚知させる。
2. **社内の全 Alpha ユーザー**: 管理者間での相互監視と、意図しない権限変更の早期発見を可能にする。

### 10.2 通知の内容と透明性
通知メッセージには、**「誰が」変更を行ったのか（実行者名）**を必ず含める。
- **例**: 「[実行者名] さんによって、あなたのロールが [旧ロール] から [新ロール] に変更されました。」
- **目的**: 誰が権限操作を行ったかを明確にすることで、ガバナンスの透明性を担保する。

### 10.3 最後の Alpha 保護 (再掲)
第3.5節で定義した通り、社内で唯一の Alpha ユーザーが自分自身を降格または削除しようとした場合、システムはこれを拒否し、権限委譲を促すアラートを表示する。
