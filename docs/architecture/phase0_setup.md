...
### Colima (Native Container Runtime)
Docker Desktop に代わる、Apple Silicon 機向けの軽量・高速な実行環境です。
* **入手元:** `/opt/homebrew/bin/brew install colima docker`
* **設定:** `colima start --cpu 2 --memory 2` (VZ 仮想化ドライバの利用を推奨)
* **役割:** エージェントが安全にコードを実行するための隔離空間（Sandbox）を提供します。
...
### Step 2: Colima の起動確認
コンテナ環境を立ち上げ、Docker コマンドが `aarch64` (arm64) として認識されるか確認します。
```bash
