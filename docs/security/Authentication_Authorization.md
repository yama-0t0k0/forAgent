...
| キー | 値の例 | 説明 |
| :--- | :--- | :--- |
| `role` | `"admin"`, `"corporate-alpha"`, `"corporate-beta"`, `"corporate-gamma"`, `"individual"` | アプリケーションの基本利用区分 |
| `canCreateCompany` | `boolean` | 法人プロファイルを作成・所有する権限の有無（一回限り） |
...
| **法人招待・権限付与時** | `canCreateCompany` | Admin画面 → Firestore | 管理者がメアド検索で既存ユーザーを特定し直接付与、または新規招待発行 |
| **法人アカウント作成完了時** | `role` (corporate-alpha), `companyId` | Cloud Functions / API | 法人プロフィール作成成功時に自動的にロールを格上げ。同時に `canCreateCompany` を `false` に変更。 |
...
本番環境のデータとテスト用データを明確に区別し、誤操作による実データ削除等を防ぐため、以下の命名規約を厳守してください。
...
> [!IMPORTANT]
> **リリース前チェックリスト**:
> アプリの正式リリース前（または環境クリア時）には、`adminId` / `companyId` / `id_individual` が上記のテスト用範囲（A9*, B9*, C0000*）に該当するドキュメントを全件手動またはスクリプトで削除する必要があります。
...
本プロジェクトの健全な運営とセキュリティを担保するため、以下のルールを「変更不可な基本原則」として定義します。

### 9.1 法人プロファイル作成の「1回限り」制限

1つの個人アカウント（Individual）から新規に法人アカウントを設立できるのは、**現所属企業で1回のみ**です。（転職して新しい会社で法人アカウントを作りたい場合は、また同じフローとなるので、その旨を運営に伝えてください）

*   **制限の実装**: 法人情報の登録が完了し、`corporate-alpha` ロールが付与された時点で、Custom Claims の `canCreateCompany` フラグは永久に `false` となり、Firestore上のフラグも同期されます。
*   **目的**: 法人のなりすまし作成や、不適切な法人アカウントの乱立を物理的に制限し、プラットフォームの信頼性を維持するため。
*   **例外措置**: 誤登録や社名変更など、正当な理由がある場合はシステム管理者（Admin）による手動介入が必要となります。

### 9.2 法人ロール体系（α・β・γ）とその統合

法人ユーザーは、その職能と責任に応じて以下の3階層に厳格に分類されます。これらはシステム全体（Firestore Rules / UI Routing / Backend Logic）で一貫して使用されます。

| ロール名 | 別名 | 権限範囲 |
| :--- | :--- | :--- |
| `corporate-alpha` | **α：採用管理者** | 法人オーナー階層（1社につき最大3名まで）。企業情報の完全な管理、メンバーの招待・削除・権限変更、全データの閲覧権限を持つ。 |
| `corporate-beta`  | **β：採用関係者** | 採用実務者。求人の作成、応募者とのやり取り、選考プロセス管理が可能。企業情報の変更は不可。 |
| `corporate-gamma` | **γ：一般社員** | 非採用実務者。自社求人の閲覧やリファラル協力は可能だが、採用管理画面へのアクセス権限を持たない。 |

> [!IMPORTANT]
> **「α」以外のユーザー（β, γ）は、法人アカウントを新規作成する権利を持ちません。** 
> したがって、これらのユーザーには最初から「法人を設立する」ボタンは表示されず、招待コード入力画面からも排除されます。

### 9.4 採用管理者の冗長化とガバナンス保護

実務上の意思決定の迅速化と、管理者の突発的な離脱（退職・異動）によるロックアウトリスクを回避するため、以下のガードレールを設けます。

1.  **最大3名体制**: 1社につき `corporate-alpha` ロールを持てるユーザーは最大3名に制限します。
2.  **最後の1人の保護 (Lockout Prevention)**: 社内に `corporate-alpha` が1名しかいない場合、そのユーザーの「退会」「削除」「ロール変更」はシステムによって強制的にブロックされます。この際、システム側でエラー（警告）を表示し、組織の管理権限を維持するために「後任となる別のユーザーを Alpha に指定（昇格）」することを具体的に促します。
3.  **冗長化の推奨通知**: 法人アカウント作成直後、および Alpha が 1 名のみの状態が半年以上続く場合、システムより「管理者冗長化（2名以上）の推奨」メッセージが採用管理者へ通知されます。
4.  **作成者の権限**: 法人を新規設立（登記）した最初のユーザーは自動的に最初の Alpha となり、同時に `canCreateCompany` 権限を永久に失効します。

### 9.5 招待権限と新規作成の運用原則

1. **招待コードの発行制限**:
   - `admin` ロールユーザーのみが未登録の「非ユーザー」に対する完全な招待権限（法人用・個人用ともに発行可能）を持っています。
   - `individual`（個人ユーザー）は、原則として招待権限を持ちません。ただし Admin から権限を付与された場合に限り、「個人用」の招待コードのみを発行可能です。法人の Alpha 等も、自身に招待権限が付与されていなければ「非ユーザー」への招待コードは出せません。
2. **既存ユーザーからの法人作成**:
   - すでに当サービスに個人ユーザーとして登録済みの状態から、新たに自分がオーナーとなる法人を立ち上げたい（法人ユーザーを新規登録したい）場合、自身で自由に法人作成ボタンを押すことはできません。
   - この場合、Admin に手動で「採用管理者（Alpha）」に対する権限付与措置（具体的には `canCreateCompany` などを操作）を依頼する必要があります。Adminからの付与が完了すると、マイページ上に「法人アカウントを新規作成する」手段が表示されます。
