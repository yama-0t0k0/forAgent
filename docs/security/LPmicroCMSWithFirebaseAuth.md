...
- `apps/lp_app/app.json` の `ios.associatedDomains` に `webcredentials:<RP_ID>` が設定されていること
  - 現在の設定例（本番）: `webcredentials:latcoltd.net`
  - 移行期間の互換（必要な場合）: `webcredentials:engineer-registration-lp.web.app` を追加で残す

**バックエンド（Cloud Functions）**
- `apps/functions/src/passkey.js` の `PASSKEY_RP_ID` と `EXPECTED_ORIGINS` が、アプリ/ドメイン側の設定と整合していること
  - `PASSKEY_RP_ID` の既定（本番）: `latcoltd.net`
  - `EXPECTED_ORIGINS` の既定（本番）: `https://latcoltd.net` および `ios:bundle-id:com.engineer.registration.lpapp`
  - 移行期間の互換（必要な場合）: `https://engineer-registration-lp.web.app` を許可リストに残す
...
- **Expo Tunnel での解決策 (Best UX)**:
  - 開発中は実機検証用として、dev 用の固定ドメイン（例: `engineer-registration-lp-dev.web.app`）を一時的に Firebase Hosting に追加し、そこに AASA を配信しておく。
