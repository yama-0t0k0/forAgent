...
- **ヒートマップ実装 (Hybrid Strategy)**: コアロジックは Dart で定義され、フロントエンド表示用に JS ポート（`HeatmapCalculator.js`）が `shared/common_frontend` に配置されています。
...
- **役割**:
    - **依存関係の一元管理**: 各アプリ（Admin, Individual, Corporate, etc.）の `dart_backend` および `shared/domain_models` の依存関係解決 (`pub get`) を一括実行します。
